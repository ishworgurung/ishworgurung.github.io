...
![ConsiderDone logo]({{ '/posts/considerdone-logo.png' | relative_url }})
...
The first version uses local-search data to find and rank candidates. It then uses an outbound voice workflow to speak with businesses in the normal channel they already use: the phone. A provider does not need to join a new marketplace or adopt a new point-of-sale system before they can take a job.
...
![ConsiderDone.au job workflow]({{ '/posts/job-states.svg' | relative_url }})
...
A booking moves through many steps: search, calling, quote collection, approval, payment, provider details, job completion, and payout. One orchestration layer coordinates those steps, while each integration stays narrowly focused on its own job.
...
For that reason, side-effecting operations use persistent idempotency keys and database constraints. The system records its progress and can resume in-flight tasks after a restart instead of treating a restart as a failed job. This is less glamorous than the voice interface, but it is essential infrastructure for a service that makes bookings and moves money.
...
For a standard job, the system first places an authorisation hold for the homeowner’s agreed spending cap. It only captures the exact quoted amount after the homeowner approves it. If the quote is declined or no provider works out, the hold is not captured.
...
considerdone.au keeps card numbers out of its servers by using a browser-based payment component. It keeps an append-only audit trail of state-changing events while storing event payloads as hashes rather than raw data. Provider payout links are random, scoped to one booking, time-limited, and single-use.
